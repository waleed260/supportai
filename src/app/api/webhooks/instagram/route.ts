...
export async function POST(request: Request) {
  try {
    ...
    if (!appSecret) {
      log.error('META_MESSAGING_APP_SECRET not configured', { route: '/api/webhooks/instagram' })
    ...
    if (!parsed.success) {
      ...
      Sentry.captureException(new Error('Malformed Instagram webhook payload'), {
        tags: { route: '/api/webhooks/instagram' },
    ...
    const { success: allowed, reset } = await limiters.webhook(`webhook:instagram:${orgId}`)
    if (!allowed) {
